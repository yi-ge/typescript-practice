 ...
 * @return {ListNode}
 */
export const reverseKGroup = function (head: ListNode | null, k: number): ListNode | null {
  // 《K个一组翻转链表》 https://www.wyr.me/post/622
  let sum = 0 // 记录进行的结点个数
  let start: ListNode | null = head // 记录每次翻转的第一个元素
  let res: ListNode | null = head // 返回值：如果进行过翻转，则为第一次翻转的最后一个结点
