export const findOrder = function (numCourses: number, prerequisites: number[][]): number[] {
  const courses = Array(numCourses).fill(0) // 初始化 上课 需要先完成课程 的门数
  const obj: any = {} // 记录受该课程 影响的其他课
  ...
  while (queue.length) {
    const cur = queue.shift() as number // 出队   表示该课已经上了
    res.push(cur) // 把出队的放入 结果数组
    const list = obj[cur] // 获取受该课影响的 课
    list && list.forEach((item: number) => {
